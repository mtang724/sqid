  ...
  <sqid-bars>
    <template v-slot:mainbar>
      ...
      <div id="hierarchy-information">
        ...
        <i18n
          tag="div"
          path="entity.superProperties"
          v-if="superProperties.length"
        >
          ...
          <ul place="properties" class="comma-separated">
            <li
              v-for="(superProperty, supidx) of superProperties"
              :key="supidx"
            >
        ...
        <i18n
          tag="div"
          path="entity.noSuperClasses"
          v-else-if="hierarchyStatistics.directSubclasses"
        >
          <b place="subclassOf"><entity-link entityId="P279"/></b>
          <span place="class">{{ label }}</span>
        </i18n>
        <i18n
          tag="div"
          path="entity.instanceClasses"
          v-if="instanceClasses.length"
        >
          <b place="instanceOf"><entity-link entityId="P31"/></b>
          <span place="instance">{{ label }}</span>
          <ul place="classes" class="comma-separated">
            <li
              v-for="(instanceClass, incidx) of instanceClasses"
              :key="incidx"
            >
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>
                <ul class="comma-separated">
                  <li
                    v-for="(exampleValue, exvidx) of exampleValues"
                    :key="exvidx"
                  >
            ...
            <tr>
              <th
                v-t="'entity.propertyTypicalProperties'"
                v-b-tooltip
              ...
              <td>
                <div class="four-lines">
                  <ul class="comma-separated">
                    <li
                      v-for="(typicalProperty, typpidx) of typicalProperties"
                      :key="typpidx"
                    >
            ...
            <tr>
              <th
                v-t="'entity.propertyStatements'"
                v-b-tooltip
                :title="$t('entity.propertyStatementsDescription')"
              />
              <td>
                <i18n path="entity.propertyStatementsValue">
            ...
            <tr>
              ...
              <td>
                <div class="four-lines">
                  <ul class="comma-separated">
                    <li
                      v-for="(qualifier, pqidx) of propertyUsage.qualifiers"
                      :key="pqidx"
                    >
      ...
      <sqid-collapsible-card
        v-if="
          hierarchyStatistics.directInstances ||
            hierarchyStatistics.allInstances
        "
          ...
          <tbody>
            <tr>
              ...
              <td>
                ...
                <div
                  class="four-lines"
                  v-if="hierarchyStatistics.directInstances"
                >
                  <ul class="comma-separated">
                    <li
                      v-for="(exInstId, eiidx) of exampleInstances"
                      :key="eiidx"
                    >
            ...
            <tr>
              <th
                v-t="'entity.allInstances'"
                v-b-tooltip
                :title="
                  $t('entity.allInstancesDescription', {
                    num: hierarchyStatistics.allSubclasses
                  })
                "
              />
            ...
            <tr>
              <th
                v-t="'entity.classTypicalProperties'"
                v-b-tooltip
              ...
              <td>
                <div class="four-lines">
                  <ul class="comma-separated">
                    <li
                      v-for="(relatedId,
                      relidx) of hierarchyStatistics.relatedProperties"
                      :key="relidx"
                    >
      ...
      <sqid-collapsible-card
        v-if="
          hierarchyStatistics.directSubclasses ||
            hierarchyStatistics.superClasses.length
        "
          ...
          <tbody>
            <tr>
              ...
              <td v-if="superClasses.length">
                <ul class="comma-separated">
                  <li
                    v-for="(superClass, supcidx) of superClasses"
                    :key="supcidx"
                  >
                    <entity-link :entityId="superClass.value.id" />
                    <sqid-qualifier-icon :claim="superClass" />
                    <b-badge>{{
                      superClassesUsage.get(superClass.value.id)
                    }}</b-badge>
                  </li>
                </ul>
              </td>
                ...
                <b-tabs>
                  <b-tab>
                    <template v-slot:title>
                      <i18n
                        path="entity.directSubclassesInstances"
                        v-b-tooltip="
                          $t('entity.directSubclassesInstancesDescription')
                        "
                      >
                        <b-badge place="count">{{
                          Object.keys(subclassesInstances).length
                        }}</b-badge>
                      </i18n>
                    </template>
                    <ul class="comma-separated">
                      <li
                        v-for="(subClass, dsciidx) of sortedSubclassesInstances"
                        :key="dsciidx"
                      >
                  ...
                  <b-tab>
                    <template v-slot:title>
                      <i18n
                        path="entity.directSubclassesSubclasses"
                        v-b-tooltip="
                          $t('entity.directSubclassesSubclassesDescription')
                        "
                      >
                        <b-badge place="count">{{
                          Object.keys(subclassesSubclasses).length
                        }}</b-badge>
                      </i18n>
                    </template>
                    <ul class="comma-separated">
                      <li
                        v-for="(subClass,
                        dscsidx) of sortedSubclassesSubclasses"
                        :key="dscsidx"
                      >
                        <entity-link :entityId="subClass.id" />
                        <b-badge>{{ subClass.count }}</b-badge>
                  ...
                  <b-tab>
                    ...
                    <ul class="comma-separated">
                      <li
                        v-for="(subClass, dscaidx) of exampleSubclasses"
                        :key="dscaidx"
                      >
    ...
    <template v-slot:sidebar>
      ...
      <div id="claims-ids" v-if="groupedClaims">
        <claim-table
          :header="$t('entity.identifierStatements')"
          :entityId="entityId"
          :claims="group('i')"
          id="identifiers"
          v-if="showGroup('i')"
...
import {
  ...
  PropertyStatistics,
} from '@/store/statistics/properties/types'
...
export default class Entity extends Vue {
  ...
  @Action private getExampleValues!: (
    entityId: EntityId,
  ) => Promise<EntityId[]>
  @Action private getExampleInstances!: (
    entityId: EntityId,
  ) => Promise<EntityId[]>
  @Action private getExampleSubclasses!: (
    entityId: EntityId,
  ) => Promise<EntityId[]>
  @classStatistics.Action private getClassHierarchyRecord!: (
    entityId: EntityId,
  ) => Promise<ClassStatistics>
  @classStatistics.Action private getClassUsageCounts!: (
    entityIds: EntityId[],
  ) => Promise<Map<EntityId, number>>
  @classStatistics.Getter private getHierarchyRecord!: (
    entityId: EntityId,
  ) => ClassStatistics
  ...
  @propertyStatistics.Action private getPropertyUsage!: (
    entityId: EntityId,
  ) => Promise<PropertyStatistics>
  @propertyStatistics.Getter private propertyGroups!: (
    entityId: EntityId,
  ) => PropertyClassification
  ...
  @Getter private getValuesForProperty!: (
    entityId: EntityId,
    propertyId: EntityId,
  ) => Array<{}>
  @Getter private getPropertyDatatype!: (
    entityId: EntityId,
  ) => WBDatatype | undefined
  ...
  private groupedReverseClaims: Map<
    PropertyClassification,
    ClaimsMap
  > | null = null
  ...
  private superClassesUsage: Map<EntityId, number> = new Map<
    EntityId,
    number
  >()
  ...
  private updateEntityData() {
    ...
    this.getClassHierarchyRecord(this.entityId).then((record) => {
      if (record) {
        this.hierarchyStatistics = record
        const entityIds = record.superClasses.concat(
          record.nonemptySubClasses,
        ...
        this.getClassUsageCounts(record.nonemptySubClasses).then((usage) => {
          for (const subclassId of record.nonemptySubClasses) {
            ...
            const subclasses = this.getHierarchyRecord(subclassId)
              .allSubclasses
    ...
    this.getExampleItems(this.entityId).then((examples) => {
      this.exampleItems = examples
    })

    this.getExampleValues(this.entityId).then((examples) => {
      this.exampleValues = examples
    })

    this.getExampleInstances(this.entityId).then((examples) => {
      this.exampleInstances = examples
    })

    this.getExampleSubclasses(this.entityId).then((examples) => {
      this.exampleSubclasses = examples
    })

    this.refreshRelatedProperties([this.entityId]).then(
      (data: RelatednessMapping) => {
        if (!(this.entityId in data)) {
          return
        }

        const related = Object.entries(data[this.entityId]).sort(
          (left, right) => {
            if (left[1] < right[1]) {
              return 1
    ...
    this.getPropertyUsage(this.entityId).then((usage) => {
      if (usage === undefined) {
        return
      }
    ...
    const forwardClaims = this.getEntityData(this.entityId).then(
      (data: {
        label: string;
        aliases: string[];
        description: string;
        ...
        if (kind === 'property') {
          this.propertyDatatype =
            this.getPropertyDatatype(this.entityId) || null

          const values =
            (this.getValuesForProperty(
              this.entityId,
              'P1647',
            ) as QualifiedEntityValue[]) || []
        ...
        this.getClassUsageCounts(
          this.superClasses.map((entity) => entity.value.id),
        ).then((counts) => {
    ...
    const reverseClaims = this.getReverseClaims(this.entityId).then(
      (claims: ClaimsMap) => {
    ...
    Promise.all([forwardClaims, reverseClaims, this.refreshClassification()])
      .then(() => {
        const properties = []
  ...
  private regroupClaims(relatednessScores: RelatednessMapping) {
    if (this.claims) {
      this.groupedClaims = groupClaims(
        this.claims,
        this.propertyGroups,
        relatednessScores,
      )
    ...
    if (this.reverseClaims) {
      this.groupedReverseClaims = groupClaims(
        this.reverseClaims,
        this.propertyGroups,
        relatednessScores,
      )
  ...
  private updateLinks() {
    const urls: Array<{ url: string; label: any }> = [
      { url: this.wikidata, label: this.$t('entity.wikidataLink') },
    ]
    ...
    // urls.push({
    //   url: this.getReasonatorUrl(this.entityId),
  ...
  private get sortedSubclassesInstances() {
    ...
    for (const [subclassId, count] of Object.entries(
      this.subclassesInstances,
    )) {
      result.push({ id: subclassId, count })
    }
