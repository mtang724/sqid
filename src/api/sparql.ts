...
export async function sparqlQueries(
  queries: string[],
  ...
  if (statements.length < MAX_RELATED_BATCH) {
    ...
    const results = await sparqlQueries(
      properties.map((propertyId) => {
        return relatingStatementsForPropertyQuery(
          entityId,
          propertyId,
          MAX_RELATED_BATCH,
        )
...
async function getRelatingStatements(
  entityId: EntityId,
  limit: number,
): Promise<SqidStatement[]> {
...
export async function getPropertySubjects(
  ...
  const result = await sparqlQuery(
    propertySubjectsQuery(propertyId, lang, entityId, limit),
  )
...
export async function getPropertyObjects(
  ...
  const result = await sparqlQuery(
    propertyObjectsQuery(propertyId, lang, entityId, limit),
  )
