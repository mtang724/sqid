...
export async function getEntities(
  ...
  for (let start = 0; start <= ids; start += MAX_ENTITIES_PER_API_REQUEST) {
    ...
    if (chunk.length) {
      chunks.push(chunk)
      // chunks now is a list of list of ids
    }
  }
...
export async function getLabels(
  ...
  const langCode = lang || i18n.locale
  const labels = new Map<string, Map<string, string>>() // language -> (id -> label)
  const nativeLabels = new Map<string, string>()
  labels.set(langCode, nativeLabels)

  for (const [entityId, entity] of Object.entries(entities)) { // return key-value pair, similar to enumerate()
...
export async function getEntityInfo(entityId: EntityId) {
  try {
    // Basically testing existence of entity and test whether its format is legal
  ...
  for (const [propertyId, theClaims] of claims) {
    ...
    for (const claim of theClaims) {
      ...
      if (
        ...
        ['wikibase-item', 'wikibase-property'].includes(
          mainsnak.datatype,
        )
      ) {
...
export function wikidataUrl(entityId: EntityId, lang?: string) {
  ...
  return `${customDomain}/entity/${entityId}${forceLang}`
}
...
export function dateFromTimeData(data: TimeDataValue) {
  ...
  // console.log(`${prefix}${year}-${month}-${day}T${hour}:${minute}:${second}${TZ}`)
  const offTime = new Date(
    `${prefix}${year}-${month}-${day}T${hour}:${minute}:${second}${TZ}`,
  )
  const correctTime = new Date( offTime.getTime() - offTime.getTimezoneOffset() * -60000 )
  // console.log(result)
  return {
    time: correctTime,
...
export function coordinateFromGlobeCoordinate(data: GlobeCoordinateValue) {
  ...
  const {
    ...
    minutes: lam,
    seconds: las,
  } = extractCoordinateComponents(Number(lat))
  ...
  const {
    ...
    minutes: lom,
    seconds: los,
  } = extractCoordinateComponents(Number(lon))
...
export function idsFromQualifiedEntity(
  entity: QualifiedEntityValue,
): EntityId[] {
